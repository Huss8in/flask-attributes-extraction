...
- FINAL SELF-CHECK before returning: (1) if Brand is non-empty, Product Name should almost always be non-empty too (Brand + the model/product words that follow it, trimmed of packaging/variant noise) — an empty Product Name next to a filled Brand is usually a MISTAKE, go back and fill it; Product Name must be strictly shorter than Item Name and never identical to it; (2) Features contains NO values that belong in Color/Size/Material/Pattern/Brand/Gender/Fashion Type; (3) any value with a unit (g/kg/ml/L/cm) is in Measurements NOT Size; (4) any age variant (Ny, Nm, Nmo, N years, N months) is in Age NOT Size; (5) Generic Name is NOT simply a copy of the Item Category value — it must be the specific product noun (e.g. "shirt" not "top", "frying pan" not "pot and pan"). If ANY of these fail, fix it before outputting.
...
- Brand: the brand/manufacturer/label name — usually the leading proper-noun word(s) of the Item Name (e.g. "Astonish Premium Starch Spray" -> Astonish; "White Magic Jumbo Kitchen Rolls" -> White Magic; "Tescoma Meat Fork Presto" -> Tescoma; "Apple iPhone 17" -> Apple). A brand is a recognizable company/label name, NOT a generic descriptor, NOT a vendor/store name, NOT a marketing word. LEAVE EMPTY when:
  ...
  - The leading word looks like a store/vendor name, not the manufacturer
  - You are unsure whether it's a real brand — when in doubt, leave EMPTY rather than guessing.
- Gender: ALWAYS fill this for fashion, footwear, apparel, swimwear, and accessories. Infer it FIRST from the Menu Category, then the Shopping Category, Item Category, Item Name, or the images (e.g. Menu Category "Women Shirts" -> Women; "Boys Basic Tee" -> Boys; a sports bra -> Women). The Menu Category is the strongest gender signal — never contradict it (if it says "Women", the Gender is Women, not Men). Choose EXACTLY ONE of these SIX values — no other value is acceptable:
  - "Women" — adult women's product (e.g. women's dress, sports bra, maternity wear)
  - "Men" — adult men's product (e.g. men's shirt, men's cologne, beard trimmer)
  - "Unisex women, Unisex men" — ADULT product that genuinely suits both adult women AND adult men (e.g. plain unisex t-shirt, headphones, smart watch, sunglasses)
  - "Girls" — kids product for girls (e.g. girl's dress, hairband for girls)
